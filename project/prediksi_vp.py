...
st.markdown('### Prediksi Volume Produksi untuk 3 Tahun Mendatang')

# Dapatkan jalur relatif ke file model dan data
current_dir = os.path.dirname(os.path.abspath(__file__))
model_path = os.path.join(current_dir, 'best_sarima_model.pkl')
data_path = os.path.join(current_dir, 'dataset.xlsx')

# Memuat model SARIMA terbaik
best_model_fit = load_model(model_path)

# Tanggal awal untuk prediksi
start_date = pd.Timestamp('2023-06-30')

# Prediksi 120 bulan ke depan
predictions, future_dates = predict_future(best_model_fit, start_date, 36)

# Memuat data asli dari file Excel dan meresample ke bulanan
monthly_data = load_and_resample_data(data_path)
...
# Komponen Streamlit
year_to_predict = st.slider('Tentukan Bulan:', min_value=1, max_value=36, value=5)
